...
const getTotals = (array: [ShotgroupEntry]): ShotTotals => {
  ...
  const total_three_fg = parseInt(
    ((total_threes_made / total_threes_attempted) * 100).toFixed(0)
  );
  const total_ft_percentage = parseInt(
    ((total_fts_made / total_fts_attempted) * 100).toFixed(0)
  );
